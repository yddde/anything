...
@Client.on_message(command(["playlist", f"playlist@{BOT_USERNAME}", "القائمه", f"queue@{BOT_USERNAME}"]) & other_filters)
async def playlist(client, m: Message):
   chat_id = m.chat.id
   if chat_id in QUEUE:
      chat_queue = get_queue(chat_id)
      if len(chat_queue)==1:
         await m.reply(f" **✶ الاغاني الي قيد التشغيل حالياً: **\n\n• [{chat_queue[0][0]}]({chat_queue[0][2]}) | `{chat_queue[0][3]}`", reply_markup=keyboard, disable_web_page_preview=True)
      else:
         QUE = f" **✶ قيد التشغيل حاليآ:**\n\n• [{chat_queue[0][0]}]({chat_queue[0][2]}) | `{chat_queue[0][3]}` \n\n**📖 قائمة الانتظار:**\n"
         l = len(chat_queue)
         for x in range (1, l):
            han = chat_queue[x][0]
            hok = chat_queue[x][2]
      ...
      await m.reply(" **✶ لا يوجد شيء قيد التشغيل حاليآ**")
