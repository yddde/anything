...
from config import UPDATES_CHANNEL, ALIVE_NAME, OWNER_NAME, BOT_USERNAME as bn
...
@Client.on_message(filters.command(["تحميل", "تنزيل", "بحث"], prefixes=f""))
def song(_, message):
    ...
    try:
        ...
        rep = f"**[𝐁𝐨𝐭 𝐂𝐡𝐚𝐧𝐧𝐞𝐥♬⁩](t.me/{UPDATES_CHANNEL})\n[{ALIVE_NAME}](t.me/{OWNER_NAME})**"
...
@Client.on_message(
    command(["vsong", f"vsong@{bn}", "فيديو", f"video@{bn}"]) & ~filters.edited
)
...
async def lyrics(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("**✶ قم بإرسال اسم المقطع**")
            return
